...
void print_tag(void)
{
    ...
    sprintf(buffer, "[BARBER | TIME: %ld | PID: %d]: ", microseconds, getpid());
    puts(buffer);
}
...
void handle_clients()
{
    while(!barber_interrupted)
    {
        ...
        if(shared->current_clients == 0 && shared->b_state == WORKING && shared->at_chair == -1)
        {
            ...
            print_tag(); printf("The barber is going to sleep\n");
            sem_give(shared_semid, 1);
            sem_take(queue_semid, 1);
            print_tag(); printf("The barber is waking up\n");
        }
        else if(shared->b_state == WORKING && shared->at_chair != -1)
        {
            sem_take(chair_semid, 1);
            print_tag(); printf("The client %d has sat down. The barber begins "
                "to shave him\n", shared->at_chair);
            print_tag(); printf("The barber has finished shaving the client %d\n",
                shared->at_chair);
            shared->at_chair = -1;
        ...
        else if(shared->b_state == WORKING)
        {
            ...
            shared->head++;
            print_tag(); printf("The barber is inviting the client %d\n",
                shared->at_chair);
